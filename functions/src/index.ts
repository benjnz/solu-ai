...
const getGeminiModel = (name: string = MODEL_NAME, options?: any) => {
    const key = functions.config().gemini?.key || process.env.GEMINI_API_KEY;
    if (!key) {
        throw new Error("Gemini API key is not configured.");
    }
    const genAI = new GoogleGenerativeAI(key);
    return genAI.getGenerativeModel({ model: name, ...options });
};
...
export const refineBlueprint = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "The function must be called while authenticated.");
    }
...
export const executeAgentCommand = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "The function must be called while authenticated.");
    }
    ...
    try {
        ...
        const agentIntelligence = config.agent_intelligence || { knowledgeGraphEnabled: true, syncFrequency: 'Real-time', influenceWeight: 0.5 };
        ...
        const fleetVariance = config.fleet_variance || { agentVarianceEnabled: true, testGroupSize: 5, championshipFrequency: 'Daily' };
        const hierarchy = config.fleet_hierarchy || { parentAgentID: '', subAgentDelegationEnabled: true, recursionLimit: 3 };
...
- Agent Context: ${config.sharedState ? JSON.stringify(config.sharedState) : 'Isolated'}
- Blueprint Requirements: ${JSON.stringify(blueprintContext || {}, null, 2)}

**Knowledge & RAG Strategy:**
- Content Ingestion: ${ragConfig.sources?.length > 0 ? ragConfig.sources.map((s: any) => `${s.name} (${s.type})`).join(', ') : 'No specialized repositories linked.'}
...
**Agent Intelligence & Shared Knowledge Graph (SKG):**
- Knowledge Graph: ${agentIntelligence.knowledgeGraphEnabled ? 'ACTIVE' : 'DISABLED'}
- Sync Frequency: ${agentIntelligence.syncFrequency}
- Influence Weight: ${agentIntelligence.influenceWeight * 10}/10
...
- Agent Variance: ${fleetVariance.agentVarianceEnabled ? 'ACTIVE' : 'DISABLED'}
...
- Parent Agent: ${hierarchy.parentAgentID || 'ORCHESTRATOR-ROOT'}
- Sub-Agent Delegation: ${hierarchy.subAgentDelegationEnabled ? 'ACTIVE' : 'DISABLED'}
...
${eventTriggers.length > 0 ? eventTriggers.map((t: any) => `- ${t.type} from ${t.source}: ${t.action}`).join('\n') : '- No active external event triggers.'}

**Available Connections:**
${config.connections?.length > 0 ? config.connections.map((c: any) => `- ${c.name} (${c.type}): ${c.details ? JSON.stringify(c.details) : 'Generic'}`).join('\n') : '- Simulation mode only'}

**Available Strategic Tools:**
${config.tools?.length > 0 ? config.tools.map((t: any) => `- ${t.name} (Runtime: ${t.runtime || 'Default'}): ${t.description}`).join('\n') : '- No custom tools registered.'}
...
`
        ...
        if (agentId !== 'preview-instance') {
            ...
            await db.collection('deployed_agents').doc(agentId).set({
                ...
                totalTasks: admin.firestore.FieldValue.increment((jsonResponse.logs?.length || 0) + 1),
                lastLatency: latency,
